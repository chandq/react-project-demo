...
class _Modal extends Component {
	constructor(props) {
		...
		if (props.fade !== 'edit' && props.type === 'tab') {
			requiredForm = children.map((_, i) => {
				return {[i]: _.props.required}
			})
	...
	updateCurrentStepData = ({pageData = {}, nextStepAllowed = true, previousStepAllowed = true, submitAllowed = true}) => {
		this.setState({
			currentPageData: Object.assign({}, pageData),
			nextStepAllowed,
			previousStepAllowed,
			submitAllowed
		});
	...
	handleSubmit = e => {
		...
		form.validateFields((err, values) => {
			...
			if (!err) {
				...
				if (type === 'step') {
					values = Object.assign(values, Object.values(formStore).reduce((pre, next) => {
				...
				<Tabs type="card" onChange={this.tabChange}>
					{
						React.Children.map(this.props.children, (ele, index) => {
							return <TabPane key={ele.title || index}
							                tab={ele.props.required ? `* ${ ele.props.title}` : ele.props.title}>
								{React.cloneElement(ele, {
									form: this.props.form,
									updateCurrentStepData: this.updateCurrentStepData
								})}
							</TabPane>
						})
					}
		...
		if (type === 'step') {
			_steps = React.Children.map(this.props.children, (ele, index) => {
				return Object.assign({}, {
					...
					content: React.cloneElement(ele, {
						form: this.props.form,
						clear: this.clearData,
						_data: this.state.formStore,
						updateCurrentStepData: this.updateCurrentStepData
					})
				})
			})
			_footer = [
				current > 0 && <Button key="previous" bgColor={{bgcolor: "#019bfd"}} style={{marginLeft: 8}}
				                       disabled={!this.state.previousStepAllowed}
				                       onClick={() => this.prev()}> 上一步 </Button>,
				current < _steps.length - 1 &&
				<Button bgColor={{bgcolor: "#019bfd"}} key="next" disabled={!this.state.nextStepAllowed}
				        onClick={() => this.next()}>下一步</Button>,
				<Button key="cancel" onClick={this.handleCancel} b
				        gColor={{bgcolor: "#002e4c"}}>取消</Button>,
				current === _steps.length - 1 && <Button key="submit" htmlType="submit" bgColor={{bgcolor: "#019bfd"}}
				                                         disabled={!this.state.submitAllowed}
				                                         form={formName}>确定</Button>,
			]
		} else {
			_footer = [
				<Button key="cancel" onClick={this.handleCancel} bgColor={{bgcolor: "#002e4c"}}>取消</Button>,
				<Button key="submit" htmlType="submit" bgColor={{bgcolor: "#019bfd"}} form={formName}
				        disabled={!this.state.submitAllowed} onClick={this.handleOk}>确定</Button>,
			]
		}
		return (
